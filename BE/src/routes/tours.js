...
router.get("/", async (req, res) => {
  ...
  const [rows] = await pool.query(
    "SELECT id, code, title, price, duration_days, status FROM tours LIMIT ? OFFSET ?",
    [perPage, offset]
  );
...
router.get("/:id", async (req, res) => {
  ...
  const [rows] = await pool.query(
    ...
    FROM tours t
    JOIN locations l ON t.main_location_id = l.id
    WHERE t.id = ?`,
    [id]
  );
...
router.post(
  "/",
  authenticate,
  authorize([1 /* example role_id for admin */]),
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [row] = await pool.query("SELECT * FROM tours WHERE id = ?", [
        result.insertId,
      ]);
...
router.put("/:id", authenticate, authorize([1]), async (req, res) => {
  ...
  await pool.query(
    "UPDATE tours SET title = COALESCE(?, title), price = COALESCE(?, price), status = COALESCE(?, status), updated_at = CURRENT_TIMESTAMP WHERE id = ?",
